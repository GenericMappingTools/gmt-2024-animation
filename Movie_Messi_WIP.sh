...
##	c. Graficar zoom Europa W
    ...
    gmt grdimage  @earth_day -R\${REGION2} -J\${PROJ2} -X\${X} -Y\${Y2} -Bf --FRAME-TYPE=plain
    gmt coast -Df -N1/thinnest
...
#	gmt movie main.sh -Iin.sh -Sbpre.sh -C24cx10.46cx80 -Ttimes.txt -NMovie_Messi_v2  -H2 -D24 -Ml,png -Vi -Gblack  \
   	gmt movie main.sh -Iin.sh -Sbpre.sh -C${W}cx${H}cx80 -Ttimes.txt -NMovie_Messi_WIP  -H2 -D24 -Ml,png -Vi -Gblack  \
    -Lc0+jTR+o0.3/0.3+gwhite+h+r --FONT_TAG=14p,Helvetica,black --FORMAT_CLOCK_MAP=- --FORMAT_DATE_MAP=dd-mm-yyyy       \
	-Lc1+jTL+o0.3/0.3+gwhite+h+r -Zs #  -W -Fmp4 #-Pf+ac0 #+jRM+w5.9c+o2.7/0.8c+P3,white+p1,red+a1+f11p,2,white -K+po

# Place animation
mkdir -p mp4
mv -f Movie_Messi_WIP.mp4 mp4
mkdir -p png
mv -f Movie_Messi_WIP.png png
